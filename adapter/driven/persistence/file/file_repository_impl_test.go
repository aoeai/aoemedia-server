...
func newDomainFile(t *testing.T) *domainFile.DomainFile {
	...
	builder := domainFile.NewMetadataBuilder()
	metadata := builder.FileName("test.txt").StoragePath(path).Source(1).
...
func assertSavedSuccess(t *testing.T, id int64, file *domainFile.DomainFile) {
	...
	// 验证各个字段是否正确保存
	assert.Equal(t, file.Content().Hash(), savedFile.Hash, "文件哈希值不匹配")
	assert.Equal(t, file.Content().SizeInBytes(), savedFile.SizeInBytes, "文件大小不匹配")
	assert.Equal(t, file.Metadata().FileName(), savedFile.Filename, "文件名不匹配")
	assert.Equal(t, file.Metadata().StoragePath(), savedFile.StoragePath, "存储路径不匹配")
	assert.Equal(t, file.Metadata().Source(), savedFile.Source, "文件来源不匹配")
}
